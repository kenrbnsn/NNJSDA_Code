...
$subject = 'Upload NNJSDA Documents';
$ver = '(v1.0.2)';
$pageaddr = 'upload.nnjsda.documents';
$show_files = true;
$errors = array();
$msg = '';
...
$valid_uploads = array('pdf' => array('application/force-download','application/pdf'),
	...
	if (!empty($errors))
		...
		if (is_uploaded_file($_FILES['filespec']['tmp_name'])) {
			...
			$uploaddir = '../nnjsda_documents/';
			$uploadfile = $uploaddir . basename($_FILES['filespec']['name']);
			$q = "select * from documents where doc_filename like '%" . mysql_real_escape_string(basename($_FILES['filespec']['name'])) . "%'";
			...
			if (mysql_num_rows($rs) > 0) {
				...
				$q = "insert into documents set " . implode(', ',$qtmp);
				$rs = mysql_query($q) or die("Problem with the query: <span style='color:red'>$q</span> on line " . __LINE__ . '<br>' . mysql_error());
				...
				mail('kenrbnsn@nnjsda.org','NNSJDA Document Uploaded',strip_tags($msg) . "\nby: " . $_POST['upload_who'] . ' (' . $_POST['upload_email'] . ')','From: NNJSDA Docs Uploaded <' . $pageaddr . '@nnjsda.org>','-f ' . $pageaddr . '@nnjsda.org');
			}
		}
	}
}
