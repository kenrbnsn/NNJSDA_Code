...
function put_row($n,$l='')
{
	...
	$line[] = '	<span class=formw><input type="text" name=' . $n . ' class="textinp" value="' . urldecode($row[$n]) . '"></span>';
	$line[] = '</div>';
	echo implode("\n",$line)."\n";
}
...
function put_row_date($n,$l='')
{
	...
	$line = array();
	$loc = ($l != '')?$l:ucwords($n);
	$dt = ($row[$n] != '0000-00-00 00:00:00')?date('F j, Y g:i a',strtotime($row[$n])):'';
	$line[] = '<div class=row>';
	$line[] = '	<span class=label2>' . $loc . ':</span>';
...
if (isset($_POST['submit']))
	switch ($_POST['submit']) {
		case 'Update Class':
			...
			if($result) $msg[] = 'Event updated OK';
			else $err_msg[] =  '<span class=error>Problem updating event:'.mysql_error().'</span>';
		break;
		}
