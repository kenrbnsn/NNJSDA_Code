...
function disp_class_day_name($td,$day)
{
        global $cluborg, $level;
...
function check_for_position($p, $n)
{
        ...
        $body[] = '$query: ' . $query;
        $result = mysql_query($query) or die(mysql_error());
		$body[] = 'Number rows returned: ' . mysql_num_rows($result);
        mail('kenrbnsn@rbnsn.com','In check_for_position',implode("\n",$body),'From: admin trace<admin.trace@nnjsda.org>',
        		'-f admin.trace@nnjsda.org');
...
{
        global $row;
	$val = (substr($_GET['todo'],0,3) == 'add')?'':urldecode($row[$n]);
        $line = array();
        $loc = ($l != '')?$l:ucwords($n);
        $line[] = '<div class=row>';
...
function put_row_date($n,$l='')
{
        ...
        $line = array();
        $loc = ($l != '')?$l:ucwords($n);
        if (substr($_GET['todo'],0,3) == 'add') $dt = '';
...
if (isset($_POST['submit']))
		...
		break;
        case "Delete Club":
                $query = "delete from clubs where id = '" . $_POST['id']. "'";
                $result = @mysql_query($query);
                if (!$result) {
...
<li><a href="#">Club</a>
        <ul style="width:5em">
                <li><a href="<?php echo $_SERVER['PHP_SELF'] ?>?todo=addclub">Add</a></li>
                <li><a href="<?php echo $_SERVER['PHP_SELF'] ?>?todo=updclub">Update</a>
                        <ul>
                                <?php disp_club_names('upd') ?>
                        </ul>
                </li>
                <li><a href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=delclub>Delete</a>
                        <ul>
                                <?php disp_club_names('del') ?>
                        </ul>
                </li>
                <li><a href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=dispclub>Display</a>
                        <ul>
                                <?php disp_club_names('disp') ?>
                        </ul>
                </li>
        </ul>
</li>
<li><a href="#">Class</a>
        <ul style="width:5em">
                <li><a href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=addclass>Add</a></li>
                <li><a href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=updclass>Update</a>
                        <ul>
                                <?php disp_class_names('upd') ?>
                        </ul>
                </li>
                <li><a href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=delclass>Delete</a>
                        <ul>
                                <?php disp_class_names('del') ?>
                        </ul>
                </li>
                <li><a href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=dispclass>Display</a>
                        <ul>
                                <?php disp_class_names('disp') ?>
                        </ul>
                </li>
        </ul>
</li>
<li><a <?php echo $teststyle ?> href="#">Contacts</a>
        <ul style="width:5em">
                <li><a href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=addcontacts>Add</a>
                        <ul>
                                <?php disp_contact_boxes('add'); ?>
                        </ul>
                </li>
                <li><a href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=updcontacts>Update</a>
                        <ul>
                                <?php disp_contact_boxes('upd'); ?>
                        </ul>
                </li>
                <li><a href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=delcontacts>Delete</a>
                        <ul>
                                <?php disp_contact_boxes('del'); ?>
                        </ul>
                </li>
                <li><a href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=dispcontacts>Display</a>
                        <ul>
                                <?php disp_contact_boxes('disp'); ?>
                        </ul>
                </li>
        </ul>
</li>
<li><a <?php echo $teststyle ?> href="#">Events</a>
        <ul style="width:5em">
                <li><a href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=addevent>Add</a>
                        <?php put_months_days("addevent"); ?>
                </li>
                <li><a href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=updevent>Update</a>
                        <?php put_months_days("updevent"); ?>
                </li>
                <li><a href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=delevent>Delete</a>
                        <?php put_months_days("delevent"); ?>
                </li>
                <li><a href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=dispevent>Display</a>
                        <?php put_months_days("dispevent"); ?>
                </li>
        </ul>
</li>
<li><a <?php echo $teststyle ?> href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=adduser>Add New User</a></li>
<li><a <?php echo $teststyle ?> href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=chgpwd>Change Password</a></li>
<li><a <?php echo $teststyle ?> href="<?php echo $_SERVER['PHP_SELF'] ?>">Refresh Screen</a></li>
<li><a <?php echo $teststyle ?> href=<?php echo $_SERVER['PHP_SELF'] ?>?todo=logout>Logout</a></li>
</ul>
...
</div><form class=cp method=post action="<?php echo $PHP_SELF; ?>?todo=changepwd">
<div class=row>
...
case 'addevent':
                ...
                <span class=formw><?php put_day($eventdate); ?></span>
                </div>
                <div class=row>
...
case 'updevent':
        if (!isset($_REQUEST['eventorg'])) {
                ...
                $query = "Select * from clubs where club = '" . urlencode($event->event_org) . "'";
                $result = mysql_query($query);
                $club_info = mysql_fetch_object($result);
                $ek = array_keys($event_ar);
                if (!isset($_POST['submit'])) { ?>
                <form method=post action="<?php echo $_SERVER['PHP_SELF'] ?>">
                <input type=hidden name=id value=<?php echo $event->ind; ?>>
                <input type="hidden" name=todo value="<?php echo $todo ?>">
                <input type="hidden" name=year value="<?php echo $_REQUEST['year'] ?>">
                <input type="hidden" name="eventdate" value="<?php echo $_REQUEST['eventdate']?>">
                <input type="hidden" name="eventorg" value="<?php echo $_REQUEST['eventorg']?>">
                <div class=row>
                        <span class=label>Date:</span>
                        <span class=formw>
                        <select name=event_month size=1>
                        <option value=""></option>
                        <?php put_months($_REQUEST['eventdate']); ?>
                        </select>&nbsp;
                        <select name=event_day size=1>
                        <option value=""></option>
                        <?php
                ...
                <span class=label>Club or Organization Name:</span>
                <span class=formw><select class=textinp name="event_org" size="1">
                <?php get_club_names(urldecode($event->event_org)); ?>
                <option value="NNJSDA" <?php $s = ($event->event_org == "NNJSDA")?"selected":""; echo $s ?>>NNJSDA</option>
                <option value="5 Clubs" <?php $s = ($event->event_org == "5 Clubs")?"selected":""; echo $s ?>>5 Clubs</option>
                </select>
                </span>
                </div>
                <div class=row>
                <span class=label>Event Type:</span>
                <span class=formw><select class=textinp name=event_type size=1>
                <?php get_event_types($event->event_type); ?>
                </select>
                </span>
                </div>
                <div class=row>
                <span class=label>Event Name:</span>
                <span class=formw><input name=event_name class=textinp type="Text" value="<?php echo urldecode($event->event_name); ?>"></span>
                </div>
                <div class=row>
                <span class=label>Event Description:</span>
                <span class=formw><textarea class=textinp rows="10" name="event_description"><?php echo urldecode($event->event_description); ?></textarea></span>
                </div>
                <div class=row>
                <span class=label>Event URL:</span>
                <span class=formw><input name="event_url" type="text" class=textinp value="<?php echo eitheror(urldecode($event->event_url),$club->url); ?>"></span>
                </div>
                <div class=row>
                <span class=label>Day:</span>
                <span class=formw><?php put_day($event->event_date); ?></span>
                </div>
                <div class=row>
                <span class=label>Start Time:</span>
                <span class=formw><select name="starthour" size="1">
                ...
                <?
                 ...
                 ?>
                        <option value="00" <?php echo $sela; ?>>00</option>
                        <option value="30" <?php echo $selp; ?>>30</option>
                </select>&nbsp;<select name="startampm" size="1">
                <?
                        $sela = (date("a",strtotime($event->event_start_time)) == "am")?"selected":"";
                        $selp = (date("a",strtotime($event->event_start_time)) == "pm")?"selected":"";
                 ?>
                        <option value="am" <?php echo $sela; ?>>am</option>
                        <option value="pm" <?php echo $selp; ?>>pm</option>
                </select></span>
                </div>
                <div class=row>
                <span class=label>End Time:</span>
                ...
                <div class=row>
                <span class=label>Location:</span>
                <span class=formw><input name=event_location class=textinp type="Text" value="<?php echo  eitheror(urldecode($event->event_location),$club_info->location); ?>"></span>
                </div>
                <div class=row>
                <span class=label>Address:</span>
                <span class=formw><input name=event_address class=textinp type="Text" value="<?php echo  eitheror(urldecode($event->event_address),$club_info->address); ?>"></span>
                </div>
                <div class=row>
                <span class=label>City:</span>
                <span class=formw><input name=event_city class=textinp type="Text" value="<?php echo  eitheror(urldecode($event->event_city),$club_info->city); ?>"></span>
                </div>
                <div class=row>
                <span class=label>State:</span>
                <span class=formw><input name=event_state class=textinp type="Text" value="<?php echo  eitheror(urldecode($event->event_state),$club_info->state); ?>"></span>
                </div>
                <div class=row>
                <span class=label>Zip:</span>
                <span class=formw><input name=event_zip class=textinp type="Text" value="<?php echo  eitheror($event->event_zip,$club_info->zip); ?>"></span>
                </div>
                <div class=row>
                <span class=label>Square Dance Program:</span>
                <span class=formw>
                ...
                <div class=row>
                <span class=label>Caller(s):</span>
                <span class=formw><input name=caller class=textinp type=text value="<?php echo  eitheror(urldecode($event->event_caller),$club_info->caller); ?>"></span>
                </div>
                <div class=row>
                <span class=label>Cuer(s):</span>
                <span class=formw><input name=cuer class=textinp type=text value="<?php echo  eitheror(urldecode($event->event_cuer),$club_info->cuer); ?>"></span>
                </div>
                <div class=row>
                <span class=label>Club Contacts:</span>
                <span class=thirda  style="width:20%;"><span class=boldcu>Name:</span><br>
                ...
                $ecp=($event->event_contact_phone == "")?array("","",""):unserialize($event->event_contact_phone);
                 for ($i=0;$i<3;$i++) { ?>
                <input type=text  class=textinp name=contact_name[] value='<?php echo eitheror($ecn[$i],$cn[$i]) ?>'>
                <?php } ?>
                </span>
                <span class=thirda style="width:20%"><span class=boldcu>Email:</span><br>
                <?php for ($i=0;$i<3;$i++) { ?>
                <input type=text  class=textinp name=contact_email[] value='<?php echo eitheror($ece[$i],$ce[$i]) ?>'>
                <?php } ?>
                </span>
                <span class=thirda style="width:20%"><span class=boldcu>Phone:</span><br>
                <?php for ($i=0;$i<3;$i++) { ?>
                <input type=text  class=textinp name=contact_phone[] value='<?php echo eitheror($ecp[$i],$cp[$i]) ?>'>
                <?php } ?>
                </span>
                </div>
                <div class=row>
...
<span class=formw><?php echo urldecode($row['event_type']) ?></span>
</div>
<div class=row>
<span class=label2>Event Name:</span>
<span class=formw><?php echo urldecode($row['event_name']) ?></span>
</div>
<div class=row>
<span class=label2>Description:</span>
<span class=formw><?php echo urldecode($row['event_description']) ?></span>
</div>
<div class=row>
<span class=label2>Location:</span>
<span class=formw><?php echo urldecode($row['event_location']) ?></span>
</div>
<div class=row>
<span class=label2>Address:</span>
<span class=formw><?php echo urldecode($row['event_address']) ?></span>
</div>
<div class=row>
<span class=label2>City:</span>
<span class=formw><?php echo urldecode($row['event_city']) ?></span>
</div>
<div class=row>
<span class=label2>State:</span>
<span class=formw><?php echo urldecode($row['event_state']) ?></span>
</div>
<div class=row>
<span class=label2>Zip:</span>
<span class=formw><?php echo $row['event_zip'] ?></span>
</div>
<div class=row>
<span class=label2>URL:</span>
<span class=formw><?php echo urldecode($row['event_url']) ?></span>
</div>
<div class=row>
<span class=label2>Start Time:</span>
<span class=formw><?php echo date("g:i a",strtotime($row['event_start_time'])) ?></span>
</div>
<div class=row>
<span class=label2>Stop Time:</span>
<span class=formw><?php echo date("g:i a",strtotime($row['event_stop_time'])) ?></span>
</div>
<div class=row>
<span class=label2>Square Dance Program:</span>
<span class=formw><?php echo $row['event_sd_program'] ?></span>
</div>
<div class=row>
<span class=label2>Round Dance Program:</span>
<span class=formw><?php echo $row['event_rd_program'] ?></span>
</div>
<div class=row>
<span class=label2>Caller:</span>
<span class=formw><?php echo urldecode($row['event_caller']) ?></span>
</div>
<div class=row>
<span class=label2>Cuer:</span>
<span class=formw><?php echo urldecode($row['event_cuer']) ?></span>
</div>
<div class=row>
...
if (IsSet($clubname)) { ?>
<div id=hdr>
<h1><?php echo ucwords($stripped_club); ?></h1>
</div>
<?php      $query = "Select * from clubs where club='" . urlencode(stripslashes($_GET['clubname'])) . "' or club = '" . stripslashes($_GET['clubname']) . "'";
        $result = mysql_query ($query);
        $row = mysql_fetch_object($result);
 ?>
<div class=row>
<span class=label2>Location:</span>
<span class=formw><?php echo urldecode($row->location) ?></span>
</div>
<div class=row>
<span class=label2>Address:</span>
<span class=formw><?php echo urldecode($row->address) ?></span>
</div>
<div class=row>
<span class=label2>City:</span>
<span class=formw><?php echo urldecode($row->city) ?></span>
</div>
<div class=row>
<span class=label2>State:</span>
<span class=formw><?php echo urldecode($row->state) ?></span>
</div>
<div class=row>
<span class=label2>Zip:</span>
<span class=formw><?php echo urldecode($row->zip) ?></span>
</div>
<div class=row>
<span class=label2>County:</span>
<span class=formw><?php echo urldecode($row->county) ?></span>
</div>
<div class=row>
<span class=label2>URL:</span>
<span class=formw><?php echo urldecode($row->url) ?></span>
</div>
<div class=row>
...
<span class=formw><?php echo $row-> dance_weeks ?></span>
</div>
<div class=row>
<span class=label2>Start Time:</span>
<span class=formw><?php echo date("g:i a",strtotime($row->dance_start)) ?></span>
</div>
<div class=row>
<span class=label2>Stop Time:</span>
<span class=formw><?php echo date("g:i a",strtotime($row->dance_stop)) ?></span>
</div>
<div class=row>
<span class=label2>Square Dance Program:</span>
<span class=formw><?php echo $row->sd_program ?></span>
</div>
<div class=row>
<span class=label2>Round Dance Program:</span>
<span class=formw><?php echo $row->rd_program ?></span>
</div>
<div class=row>
<span class=label2>Caller:</span>
<span class=formw><?php echo urldecode($row->caller) ?></span>
</div>
<div class=row>
<span class=label2>Cuer:</span>
<span class=formw><?php echo urldecode($row->cuer) ?></span>
</div>
<div class=row>
<span class=label2>Contacts:</span>
<span class=thirda><span class=boldcu>Name:</span><br>
...
<?php }
			...
			<input type="hidden" name=cluborg value="<?php echo urldecode($row->club) ?>">
			<div class=row>
...
case 'updclass':
	...
	put_row('day');
        ...
        put_row('cc_email','Class Contact Email');
         ?>
        <div class=row>
                <span class=fullwidth><input type="submit" name=submit value="<?php echo $headers[$_GET['todo']] ?>"></span>
        </div>
        <div class=clearer>&nbsp;</div>
        </form>
<?}
...
case 'addclub': ?>
<div id=hdr>
<h2><?php echo $headers[$_GET['todo']] ?><br><?php echo stripslashes($_GET['clubname']) ?></h2>
</div>
<form method="post" action="<?php echo $PHP_SELF ?>">
<input type="Hidden" name=todo value=<?php echo $todo ?>>
<?php      if (!isset($_POST['submit'])) {
                if ($_GET['todo'] == 'updclub') {
                        $query = "Select * from clubs where club='" . urlencode(stripslashes($_GET['clubname'])) ."'";
                        $result = mysql_query ($query);
...
<div class=row>
<span class=label2><?php echo ucwords($club_keys[$i])?>:</span>
<span class=formw><input type=text name=<?php echo $club_keys[$i] ?> class=textinp value="<?php echo stripslashes(urldecode($row[$club_keys[$i]])); ?>"></span>
</div>
<?php } ?>
<div class=row>
<span class=label2>Dance Day:</span>
<span class=formw><?php put_day($row['dance_day'],'day') ?></span>
</div>
<div class=row>
<span class=label2>Dance Weeks:</span>
<span class=formw><input type=text name=dance_weeks class=textinp value='<?php echo $row['dance_weeks']; ?>'></span>
</div>
<div class=row>
<span class=label2>Start Time:</span>
<span class=formw><select name="starthour" size="1">
...
<span class=formw><input type=text name=caller class=textinp value='<?php  echo stripslashes(urldecode($row['caller'])); ?>'></span>
</div>
<div class=row>
<span class=label2>Club Cuer(s):</span>
<span class=formw><input type=text name=cuer class=textinp value='<?php echo stripslashes(urldecode($row['cuer'])); ?>'></span>
</div>
<div class=row>
...
$cp=($ac)?array("","",""):unserialize($row['contact_phone']);
 for ($i=0;$i<3;$i++) { ?>
<input type=text size=<?php echo $ts ?> name=contact_name[] value='<?php echo $cn[$i] ?>'><br>
<?php } ?>
</span>
<span class=third><span class=boldcu>Email:</span><br>
<?php for ($i=0;$i<3;$i++) { ?>
<input type=text size=<?php echo $ts ?> name=contact_email[] value='<?php echo $ce[$i] ?>'><br>
<?php } ?>
</span>
<span class=third><span class=boldcu>Phone:</span><br>
<?php for ($i=0;$i<3;$i++) { ?>
<input type=text size=<?php echo $ts ?> name=contact_phone[] value='<?php echo $cp[$i] ?>'><br>
<?php } ?>
</span>
</div>
<div class=row>
<span class=fullwidth><input type=submit name=submit value='<?php echo $uc_submit[$headers[$_GET['todo']]] ?>'></span>
</div>
</div>
</form>
<?php } else {
...
<form method=post action=<?php echo "$PHP_SELF?todo=$todo"; ?>&amp;action=addingcontact>
<div class=row>
        <span class=label2>Position:</span>
        <span class=formw><input type=text class="textinp" name=position <?php if (isset($_REQUEST['position'])) echo 'value="'.$_REQUEST['position'].'"';?>></span>
</div>
<div class=row>
...
<?php      $error_found = false;
        ...
        if ($error_found) {
                ...
                $query = "Insert into contacts (position,
                         ...
                         VALUES ('" . mysql_real_escape_string(trim(stripslashes($_POST['position']))) . "',
                                         '" . mysql_real_escape_string(trim(stripslashes($_POST['officer']))) . "',
                                         '" . mysql_real_escape_string(trim(stripslashes($_POST['name']))) . "',
                                         '" . mysql_real_escape_string(trim(stripslashes($_POST['address']))) . "',
                                         '" . mysql_real_escape_string(trim(stripslashes($_POST['city']))) . "',
                                         '" . mysql_real_escape_string(trim(stripslashes($_POST['state']))) . "',
                                         '" . mysql_real_escape_string(trim(stripslashes($_POST['zip']))) . "',
                                         '" . mysql_real_escape_string(trim(stripslashes($_POST['phone']))) . "',
                                         '" . mysql_real_escape_string(trim(stripslashes($_POST['email']))) . "',
                                         '" . mysql_real_escape_string($_POST['listorder']) ."')";
                $result = mysql_query($query);
                if ($result)
...
<div id=hdr>
<h1>Display Contact<br><?php echo urldecode($_REQUEST['position']); ?></h1>
</div>
<?
        $query = "Select * from contacts WHERE (position = '". urlencode(urldecode($_REQUEST['position'])) .
                "' or position = '" . mysql_real_escape_string($_GET['position']) . "') and (name='" . urlencode($_REQUEST['name']) .  "' or name='" . $_REQUEST['name'] . "')";
        $result = mysql_query($query);
        $row = mysql_fetch_object($result); ?>
<div class=row>
        <span class=label2>Position:</span>
...
case 'updcontact':
        ...
        if (!isset($_POST['submit'])) {?>
<div id=hdr>
<h1>Update Contact<br><?php echo $position; ?></h1>
</div>
<?
        $query = "Select * from contacts WHERE (position = '". urlencode(urldecode($_REQUEST['position'])) .
                "' or position = '" . mysql_real_escape_string($_GET['position']) . "') and (name='" . urlencode($_REQUEST['name']) .  "' or name='" . $_REQUEST['name'] . "')";
        $result = mysql_query($query);
        $row = mysql_fetch_object($result); ?>
        <form method=post action=<?php echo $_SERFVER['PHP_SELF']; ?>>
        <input type="hidden" name=todo value="<?php echo $_GET['todo'] ?>">
        <input type="hidden" name=ind value="<?php echo $row->ind ?>">
<div class=row>
        <span class=label2>Position:</span>
...
<div class=row>
        ...
        <span class=formw>
                <input type="radio" name="officer" value="Officer" <?php if ($row->officer == "Officer") echo "checked"; ?>>&nbsp;Officer<br>
                <input type="radio" name="officer" value="Committee Chair" <?php if ($row->officer == "Committee Chair") echo "checked"; ?>>&nbsp;Committee Chair</span>
</div>
<div class=row>
        <span class=label2>Name:</span>
...
<div class=row>
        <span class=fullwidth><input type=submit name=submit value="Update <?php echo $position ?>"></span>
</div>
<div class=clearer>&nbsp;</div>
</form>
<?php } else { ?>
<div id=hdr>
<h1>Updating Contact<br><?php echo $position; ?></h1>
</div>
<?php      $error_found = false;
        $error_msg = "";
        $tmp = array();
...
*/
        ...
        if ($error_found) {
                ...
                if (urlencode(trim(stripslashes($_POST['position']))) != $rw1->position)
                        $tmp[] = "position='" . mysql_real_escape_string(trim(stripslashes($_POST['position']))) . "'";
                if (urlencode(trim(stripslashes($_POST['name']))) != $rw1->name)
                        $tmp[] = "name='" . mysql_real_escape_string(trim(stripslashes($_POST['name']))) . "'";
                if (urlencode(trim(stripslashes($_POST['address']))) != $rw1->address)
                        $tmp[] = "address='" . mysql_real_escape_string(trim(stripslashes($_POST['address']))) . "'";
                if (urlencode(trim(stripslashes($_POST['city']))) != $rw1->city)
                        $tmp[] = "city='" . mysql_real_escape_string(trim(stripslashes($_POST['city']))) . "'";
                if (urlencode(trim(stripslashes($_POST['state']))) != $rw1->state)
                        $tmp[] = "state='" . mysql_real_escape_string(trim(stripslashes($_POST['state']))) . "'";
                if (urlencode(trim(stripslashes($_POST['zip']))) != $rw1->zip)
                        $tmp[] = "zip='" . mysql_real_escape_string(trim(stripslashes($_POST['zip']))) . "'";
                if (urlencode(trim(stripslashes($_POST['officer']))) != $rw1->officer)
                        $tmp[] = "officer='" . mysql_real_escape_string(trim(stripslashes($_POST['officer']))) . "'";
                if (urlencode(trim(stripslashes($_POST['phone']))) != $rw1->phone)
                        $tmp[] = "phone='" . mysql_real_escape_string(trim(stripslashes($_POST['phone']))) . "'";
                if (urlencode(trim(stripslashes($_POST['email']))) != $rw1->email)
                        $tmp[] = "email='" . mysql_real_escape_string(trim(stripslashes($_POST['email']))) . "'";
...
case 'delcontact':
        ...
        if (!isset($_POST['submit'])) {?>
<div id=hdr>
<h1>Delete Contact<br><?php echo $position; ?></h1>
</div>
<?
...
<div class=row>
        <span class=label2>Officer or Chair:</span>
        <span class=formw><?php echo urldecode($row->officer); ?></span>
</div>
<div class=row>
        <span class=label2>Name:</span>
        <span class=formw><?echo urldecode($row->name); ?></span>
...
<div class=row>
        <span class=fullwidth><input type=submit name=submit value="Delete <?php echo $position ?>"></span>
</div>
<div class=clearer>&nbsp;</div>
</form>
<?php } else { ?>
<div id=hdr>
<h1>Deleting Contact<br><?php echo $position; ?></h1>
</div>
<?
