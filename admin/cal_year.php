...
$subject = 'NNJSDA Admin Update Calendar';
$ver = "(2.5.5)";
$pageaddr = 'cal.year';
	...
	switch ($_POST['add_up_del_flag']) {
		case 'A':
			...
			if ($rs) {
				$tmp = array();
				$temp = array();
				$tmp[] = htmlentities(stripslashes($_POST['event_org']),ENT_QUOTES);
		...
		case 'U':
			...
			$rs = mysql_query($q) or exit("Not Ok, $q \n" . mysql_error());
			$rw = mysql_fetch_assoc($rs);
			foreach ($_POST as $fld => $val) {
				switch ($fld) {
					case 'event_date':
						if ($rw[$fld] != date('Y-m-d',strtotime($val)))
							$qtmp[] = $fld . " = '" . date('Y-m-d',strtotime($val)) . "'";
						break;
					case 'event_start_time':
					case 'event_stop_time':
						if ($rw[$fld] != date('G:i:00',strtotime($val)))
							$qtmp[] = $fld . " = '" . date('G:i:00',strtotime($val)) . "'";
						break;
					case 'evid':
					case 'add_up_del_flag':
			...
			$q = "update new_events set " . implode(', ',$qtmp) . " where ind = '" . $_POST['evid'] . "'";
			$rs = mysql_query($q);
			if ($rs)
				exit("OK");
			else
...
if (isset($_POST['evid'])) {
	...
	$form_data['url'] = str_replace('~','%7E',(eitheror('url',$event_info,$org_info)));
...
while ($rw = mysql_fetch_assoc($rs)) {
	...
	if ($rw['event_name'] != '') $tmp[] = htmlentities(stripslashes($rw['event_name']),ENT_QUOTES);
	$tmp[] = '--------------------';
...
<html>
	...
	<meta http-equiv="Content-Type" content="text/html;charset=UTF-8">
		<link type="text/css" rel="stylesheet" href="http://yui.yahooapis.com/2.9.0/build/container/assets/skins/sam/container.css">
		<link type="text/css" rel="stylesheet" href="http://yui.yahooapis.com/2.9.0/build/fonts/fonts-min.css">
		<link type="text/css" rel="stylesheet" href="http://yui.yahooapis.com/2.9.0/build/calendar/assets/calendar.css">
		<link rel="stylesheet" type="text/css" href="http://yui.yahooapis.com/2.9.0/build/button/assets/skins/sam/button.css">
		<link type="text/css" rel="stylesheet" href="http://yui.yahooapis.com/2.9.0/build/logger/assets/logger.css">
		<style>
		...
		<script type="text/javascript" src="http://yui.yahooapis.com/2.9.0/build/dragdrop/dom-min.js"></script>
		<script type="text/javascript" src="http://yui.yahooapis.com/2.9.0/build/dragdrop/event-min.js"></script>
		...
		<script type="text/javascript">
				...
				var debug_flag = new Boolean;
				debug_flag = <?php echo ($debug)?'true':'false'; ?>;
				eventSaved = false;
				link_clicked = false;
				<?php
...
//				fclose($fp);
				...
				function array_keys(ary) {
					var temp = new Array();
					var indx = 0;
					YAHOO.log(ary,'trac','array_keys, ary');
					for (var i in ary) {
					    temp[indx++] = i;
					}
					YAHOO.log(temp,'trac','array_keys, temp');
					return (temp);
				}
...
//				YAHOO.log(dump(form_data),'trac','cbGetEvent, form_data');
				...
				for (var key in form_data) {
					...
					case 'sd_program':
						...
						YAHOO.log(tmpp.length,'trac','cbGetEvent (tmpp.length) ,' + tmpp.length);
						if (tmpp.length > 0) {
					...
					case 'type':
						...
						YAHOO.log('key:['+key+']','trac','cbGetEvent, key');
						fd = fd.replace(/&/,'and');
						YAHOO.log('fd:['+fd+']','trac','cbGetEvent, form_data[' + key + ']');
						YAHOO.log('event_' + key + '_' + fd,'trac','cbGetEvent');
						$.post("<?php echo $_SERVER['PHP_SELF'] ?>", { trace: "on", key: key, fd: '[' + fd + ']', element: 'event_' + key + '_' + fd});
						if ( $('#event_' + key + '_' + fd).length && fd != '')
...
//				event_panel.setBody(tmp[2]);
	...
	renderAppt = function(workingDate, cell) {
	   ...
	   this.styleCellDefault(workingDate, cell);
		...
		evpRenderHandler = function(type,args,abj) {
			YAHOO.log('Before Render Event','trac','evpRenderHandler');
		}
	...
	addText = function(workingDate, cell) {
	   ...
	   var div = document.createElement("div");
		...
		return YAHOO.widget.Calendar.STOP_RENDER;
				...
				event_panel = new YAHOO.widget.Dialog("event_panel_markup", { width:"700px",
						  fixedcenter:false,
						  close:true,
						  draggable:true,
						  modal:false,
				...
				event_panel.beforeRenderEvent.subscribe(evpRenderHandler, event_panel, true);
	...
	<body class="yui-skin-sam">
		<div class="x">&nbsp;
			<div id="container0">
				...
				<div class="ft">
				<div id="commentbutton" style="float:right"></div>
				<div id="yearbutton1" style="float:right"></div>
				<div style="clear:both;line-height:0.01em">&nbsp;</div>
				</div>
			</div>
			<div id="container">
				<div class="hd">
				<div id="button1" style="float:right"></div>
				...
				<div class="ft">
				<div id="button3" style="float:right"></div>
				<div id="button4" style="float:right"></div>
				<div style="clear:both;line-height:0.01em">&nbsp;</div>
				</div>
			</div>
		</div>
...
<?php
	if (!isset($_SESSION['only_subdomain'])) { ?>
				 <div class="row">
				 ...
				 <?php
					$q = "select club from clubs where display_on_off = 'on' order by club";
					$rs = mysql_query($q);
					while ($rw = mysql_fetch_assoc($rs))
						echo '<option value="' . urldecode($rw['club']) . '" id="event_org_' . str_replace(array(' ','&amp;','&'),array('_','and','and'),urldecode($rw['club'])) . '">' . urldecode($rw['club']) . "</option>\n";
				?>
...
<?php } else { ?>
					<input type="hidden" id="evend_org" name="event_org" value="<?php echo $_SESSION['only_subdomain'];?>">
<?php } ?>
				 <div class="row">
