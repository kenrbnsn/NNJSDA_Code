...
function disp_club_names($td)
{
	...
	$result = mysql_query($query);
	while ($row = mysql_fetch_object($result)) {
		echo '<li style="font-size:65%;width:15em"><a href=' . $_SERVER['PHP_SELF'] . "?todo={$td}club&amp;clubname=" .  stripslashes($row->club) . ">" . stripslashes(urldecode($row->club)) . "</a></li>\n"; }
}
...
{
	global $cluborg;
	$query = "Select club_name from classes where day='$day' order by club_name";
	$result = mysql_query($query);
	while ($row = mysql_fetch_object($result)) {
		echo '<li style="width:15em"><a href=' . $_SERVER['PHP_SELF'] . "?todo={$td}class&amp;classday=" .  stripslashes($day) . "&amp;classclub=" . $row->club_name .">" . stripslashes(urldecode($row->club_name)) . "</a>\n";
		echo "</li>\n"; }
}
...
function put_day($ed)
{
	...
	for ($i=0;$i<7;$i++)
	{
		...
		echo "<input type=Radio name=event_weekday value={$days[$i]} $checked>{$days[$i]}<br>\n";
	}
}
...
function disp_contact_names($td,$pos)
{
	...
	while ($row = mysql_fetch_object($result))
	{
		echo '<li><a  href="' . $_SERVER['PHP_SELF'] . '?todo=' . $td . 'contact&amp;position=' . $pos . '&amp;name=' . $row->name . '">';
...
</div><form class=cp method=post action="<? echo $PHP_SELF; ?>?todo=changepwd">
<div class=row>
...
<div id=hdr>
<h1>Display Contact<br><? echo $_REQUEST['position']; ?></h1>
</div>
<?
	$query = "Select * from contacts WHERE position = '". urlencode($_REQUEST['position']) . "' and name='" . urlencode($_REQUEST['name']) . "'";
	$result = mysql_query($query);
	$row = mysql_fetch_object($result); ?>
<div class=row>
	<span class=label2>Position:</span>
...
<?
	$query = "Select * from contacts WHERE position = '" . urlencode($position) . "' and name='" . urlencode($name) . "'";
	$result = mysql_query($query);
	$row = mysql_fetch_object($result); ?>
...
<? 	$error_found = false;
	...
	if ($error_found) {
		...
		$query = "Update contacts set
					position='" . urlencode(trim(stripslashes($_POST['position']))) . "',
					name='" . urlencode(trim(stripslashes($_POST['name']))) . "',
					address='" . urlencode(trim(stripslashes($_POST['address']))) . "',
					city='$city',
					state='$state',
					zip='$zip',
					officer='" . urlencode(trim(stripslashes($_POST['officer']))) . "',
					phone='$phone',
					email='" . urlencode(trim(stripslashes($_POST['email']))) . "',
					listorder='$listorder' where ind='" . $_POST['ind'] . "'";
		$result = mysql_query($query);
		if ($result)
			echo "Contact <span class=bold>" . trim(stripslashes($_POST['position'])) . ', ' . trim(stripslashes($_POST['name'])) . "</span> updated successfully.";
		else {
