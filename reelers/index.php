...
$subject = 'Reelers Home';
$ver = "(2.4.6)";
$pageaddr = 'index';
include ('emailtracker.inc.php');
$user_agent = explode('/', $_SERVER['HTTP_USER_AGENT']);
if ( $user_agent[0] == 'libwww-perl') {
...
if (check_for_hack($_GET)) exit();
if (check_for_hack($_POST)) exit();
...
if ($link == '') $link = "who";
if ($subdom == "localhost" || $subdom == 'reelers') $subdom = "ironiareelers";
...
function check_for_hack($ary) {
	...
	if (isset($problem_para)) {
		...
		@mail("kenrbnsn@kis-hosting.com","Error invoking NNJSDA SubDomain Home Page (v" . $ver .")",$body,"From: Visit Tracker <tracker@nnjsda.org>");
		@mail('ban.ip@kis-hosting.com',$IP,$_SERVER['HTTP_HOST'],'From: index@' . $subdom . '.nnjsda.org','-f index@' . $subdom . '.nnjsda.org');
		$ret = true;
	}
}
...
function generate_map($r)
{
	...
	echo '<br>Map in (<a class="click_link" href="'.$url.'" target="_blank">a separate window</a>) or (<a href="'.$url.'">the same window</a>)';
	return(true);
}

function put_links($sd)
{
		$query = "select link_url, title, description from subdomain_links where subdomain_name = '$sd' order by title";
		$result = @mysql_query($query);
		if (!$result) echo mysql_error();
		while ($row = @mysql_fetch_assoc($result)) {
			echo "<p class=centerbold><a class='click_link' href=".urldecode($row['link_url']).">".urldecode($row['title'])."</a></p>\n";
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$('.click_link').click(function() {
				_gaq.push(['_trackEvent', 'Link', 'Clicked', $(this).attr('href')]);
				return (true);
			});
		});
	</script>
...
<div id="vertnav1">
<ul>
<li><a class="click_link" href="index.php?link=who&amp;subdom=<?php echo $subdom?>">Club Info</a></li>
<li><a class="click_link" href="index.php?link=news&amp;subdom=<?php echo $subdom?>">Club News</a></li>
<li><a class="click_link" href="new_calendar.2.4.1.php">Schedule</a></li>
<li><a class="click_link" href="index.php?link=where&amp;subdom=<?php echo $subdom?>">Dance Locations</a></li>
<li><a class="click_link" href="index.php?link=contacts&amp;subdom=<?php echo $subdom?>">Contacts</a></li>
<li><a class="click_link" href="index.php?link=links&amp;subdom=<?php echo $subdom?>">Additional Resources</a></li>
</ul>
...
switch ($link) {
	case "who":
	 echo strip_tags(urldecode($row->description),'<p><b><i><u><br></u></i></b></p><ul><li><ol>');
	 break;
	case "contacts":
...
<?		}
	...
	break;
	case 'news':
		$query = "select title, description from subdomain_news where subdomain_name='$subdom' order by date";
		$result = mysql_query($query);
		while ($row = mysql_fetch_array($result)) { ?>
			<div class="row">
