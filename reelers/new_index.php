...
if (!empty($_REQUEST)) {
	while (list($key, $val) = each($_REQUEST))
		if (stristr($val,'http://') !== false) $problem_para[] = $key;
	if (isset($problem_para)) {
		echo '<div style="font-size:120%;font-weight:bold;text-align:center;width:70%;margin-right: auto;margin-left: auto;border:5px double red;margin-top:0.5em;padding:0.5em;">';
		echo "You have invoked this page incorrectly.  All pertinent information has been recorded.\n";
		echo "</div>\n";
		...
		@mail("kenrbnsn@kis-hosting.com","Error invoking NNJSDA SubDomain Home Page",$body,"From: Visit Tracker <tracker@nnjsda.org>");
		exit();
	}
}
...
if (!preg_match('/slurp|[Gg]rub|[Bb]ot|archiver|NetMonitor|[Ss]qworm|crawler/', $_SERVER['HTTP_USER_AGENT'])) {
    if ($_SERVER["HTTP_X_FORWARDED_FOR"] != ""){
        $IP = $_SERVER["HTTP_X_FORWARDED_FOR"];
        $proxy = $_SERVER["REMOTE_ADDR"];
        $host = @gethostbyaddr($_SERVER["HTTP_X_FORWARDED_FOR"]);
 ...
 @mail("kenrbnsn@kis-hosting.com","NNJSDA Sub Domain $subdom version v" . $ver,$body,"From: Visit Tracker <kenrbnsn@" . $subdom . ".nnjsda.org>",
 		"-f kenrbnsn@" . $subdom . ".nnjsda.org");
}
...
switch ($link) {
	case "who":
	 echo strip_tags(urldecode($row->description),'<p><b><i><u><br></u></i></b></p>');
	 break;
	case "contacts":
