...
//			fwrite($fp,date('m/d/Y G:i a') . ' EXIF Meta data: ' . $exifp . "\r\n");
	...
	$ver = '(V2.0.2)';
	$subject = 'NNJSDA Mini Festival Picture Gallery';
	$pageaddr = 'minifestival.pictures';
...
function write_dbg($fp,$msg)
	...
	function makethumb($fn,$rt=true,$dbg=false)
	{
		...
		if ($rt) {
			...
			$tmp_name = tempnam('trace','thumb_');
   		header('Content-type: ' .image_type_to_mime_type($type));
			imagejpeg($tn,$tmp_name,100);
			readfile($tmp_name);
			unlink($tmp_name);
		}
	}
	...
	if (is_array($x)) foreach ($x as $nxt) {
		...
		if (is_dir($nxt) && basename($nxt) != 'processed' && basename($nxt) != 'donotuse' && basename($nxt) != 'thms' && basename($nxt) != 'avis' && basename($nxt) != 'trace') {
			...
			if (contains_pics($nxt,$dbg)) {
				$tmpl .= $_SERVER['PHP_SELF'] . '?d=' . urlencode($nxt) . '">';
				$pi = explode('/',$nxt);
				$tmpl .= @date('F j, Y',strtotime($pi[4]));
			}
			else {
				$tmpl .= '#">';
				$tmpl .= basename($nxt);
...
<html>
<head>
	<title>NNJSDA Mini Festival Picture Gallery</title>
...
<link rel="stylesheet" type="text/css" href="pictures.css" media="screen">
...
//    	$.post("<?php echo $_SERVER['PHP_SELF'] ?>", {trace:1, movie: movie});
			_gaq.push(['_trackEvent', 'Show', 'Movie', movie]);
...
<body>
<div id="hdr">
<h1>NNJSDA Mini Festival Picture Gallery</h1>
</div>
<div id="rest">
<?php
	$tmp = array();
	$tmp[] = '<ul id="lhc_menu" class="sf-menu sf-vertical">';
	$tmp[] = dir_list('../../pictures/minifestival',$dbg);
	$tmp[] = '<li><a href="http://www.nnjsda.org/">Back to the Home Page</a></li>';
	$tmp[] = '</ul>';
	$tmp[] =  '<div id="rhc">';
...
//			$pct = ($ow > 1024 || $oh > 1024)?50:75;
			...
			$tmp[] = '<div id="hdr">';
			$tmpx = explode('/',$v);
			$tmp[] = '<h2>' . date('l, F j, Y', strtotime($tmpx[4])) . '</h2>';
			$tmp[] = '</div>';
			$tmp[] = '<div id="rest">';
			$tmp[] = '<img style="width:' . $ow . 'px" src="' . $tf . '">';
			$tmp[] = '<hr>';
			$tmp[] = '<div class="footer">';
			$tmp[] = '<a href="' . $_SERVER['PHP_SELF'] . '?f=' . $v . '">See fullsize picture</a><br>';
...
//					if ($dbg) {
//						$exif = @exif_read_data($filename,'FILE',true);
//						echo '<pre style="text-align:left">'.print_r($exif,true).'</pre>';
//						echo '<pre style="text-align:left">'.print_r(@exif_read_data($filename,'THUMBNAIL',true,true),true).'</pre>';
//						}
		   		...
		   		$dwidth = 160;
					$dheight = 160;
		   		$tmp[] = '<a class="pic lightbox" href="?s=' . $filename .
								'" rel="test_gallery" title="' . basename($filename,'.jpg') . '"><img style="width:' . $width . 'px " src="?i=' . $filename . $dbgx .'" width="' . $width . '" height="' . $height . '"><br>' . basename($filename) . '</a>';
				}
			}
